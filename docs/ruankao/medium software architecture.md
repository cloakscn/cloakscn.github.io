...
传统的软件开发模型开发效率较低，ABSDM 模型把整个基于体系结构的软件开发过程划分为体系结构需求、设计、文档化、复审、实现和演化六个子过程。体系结构的实现过程是以复审后的文档化体系结构说明书为基础的，具体为：$分析与设计\rightarrow构件实现\rightarrow构件组装\rightarrow系统测试$。
...
管道过滤器架构风格由一系列处理单元（过滤器）组成，每个单元的输出是下一个单元的输入。过滤器负责处理数据，管道负责数据传输。如 Linux/Unix 中的 shell、传统的编译器就是管道过滤器风格。

=== "优点"

      1. 高内聚：过滤器是执行特定功能的处理服务，具有较强的内聚性；
      2. 低耦合：过滤器之间通过管道进行数据传输，过滤器之间没有直接的耦合关系；
      3. 可重用：支持过滤器的重用；
      4. 能简单的实现并发或顺序的系统；
      5. 可扩展性：容易添加新的过滤器；
      6. 灵活性：过滤器功能可重新定义，管道路线可以改变；
...
仓库风格是一种数据共享风格，主要有两类：一类是传统的数据库，另一类是黑板。数据库系统是由输入流中的事件来驱动信息处理，把执行结构存储到中央数据单元。黑板则是由中央数据单元的当前状态来驱动系统运行，用来解决状态冲突并处理可能存在的不确定性知识源。黑板常用型号处理，如语音识别、模式识别、及其翻译、句法分析等。
...
=== "缺点"
      ...
      2. 需要同步机制、加锁机制来保障数据的完整性和一致性，增大了系统设计的复杂性；
      3. 测试困难；
      4. 缺少对并行机的支持，效率低；
      5. 开发成本高；
...
软件架构复用的类型包括 **机会复用和系统复用**。机会复用是在开发过程中，只要发现有可复用的资产就复用。系统复用是在开发前进行规划，决定哪些复用。

**软件架构复用的原因：** 减少开发工作、减少开发时间、降低开发成本、提高生产力、提高产品质量，更好的互操作性。

可复用的资产包括：**需求、架构设计、元素、建模分析、测试、项目规划、过程+方法+工具、人员、样本系统、缺陷消除**。在当前的趋势下，复用体由小粒度向大粒度的方向发展。一般形式的复用包括：**函数的复用、库的复用、面向对象开发中的类、接口和包的复用**。

**软件架构复用的基本过程：$首先构建/获取可复用的软件资产（复用前提）\rightarrow管理可复用资产\rightarrow使用可复用资产$。** 
