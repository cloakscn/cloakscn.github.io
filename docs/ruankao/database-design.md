...
!!! example "我们给出一个例题来验证判定函数依赖和无损连接的不同方法"
    ...
    给定关系模式 $R<U,F>$, $U= \{ A,B,C,D,E \}$，$F=\{B \rightarrow A, D \rightarrow A, A \rightarrow E, AC \rightarrow B\}$，则 $R$ 的候选关键字为（A），分解 $\{ R1(ABCE), R2(CD) \}$（D）。

    A. CD    B. ABC    C. ACD    D.  ADE
...
对关系 $R$，如果分解成 $R1$ 和 $R2$，若 **$R1$ 和 $R2$ 的公共属性包含$R$的一个候选键**(1)，则分解是无损连接的。
{ .annotate }
   
1. **公共属性包含候选键**：意味着通过分解后的表可以通过连接操作无损地还原为原来的关系表。
...
2. 对于每个分解的表 $Ri$，将其包含的属性在矩阵中标记为 $a_i$，其他的属性标记为 $b_i$。
3. 对于任意两个子表，如果公共属性列的所有标记都可以对应相等（相同属性可以对应），则说明分解是无损连接的。
